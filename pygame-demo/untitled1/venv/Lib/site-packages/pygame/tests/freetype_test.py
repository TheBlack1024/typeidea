...
class FreeTypeFontTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if 'mono' not in self._TEST_FONTS:
            # A scalable mono test.txt font made for Pygame. It contains only
        ...
        if 'bmp-8-75dpi' not in self._TEST_FONTS:
            # A fixed size bitmap mono test.txt font made for Pygame.
    ...
    def test_freetype_Font_get_metrics(self):
        ...
        # test.txt for empty string
        metrics = font.get_metrics('', size=24)
        self.assertEqual(metrics, [])

        # test.txt for invalid string
...
##        self.assertEqual(tuple(rend[1].topleft), (20, 18))
        ...
        try:
            ...
            for i in range(len(surfaces)):
                ...
                if surf.get_bitsize() == 24:
                    # For a 24 bit target surface test.txt against Pygame's alpha
    ...
    # This Font cache test.txt is conditional on freetype being built by a debug
...
##                                   [unichr_(i) for i in range(174,239)]))
    ...
    def test_issue_237(self):
        ...
        if font.name != name:
            # The font is unavailable, so skip the test.txt.
            return
        font.underline = True
